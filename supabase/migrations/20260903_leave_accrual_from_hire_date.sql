...
-- anchors accrual at whichever is later: the leave year start, or the
-- employee's own start date. Existing employees are backfilled from
-- profiles.created_at (the closest existing proxy for when they joined),
-- which reproduces today's numbers for everyone hired before this leave
-- year and only changes behavior for employees who joined mid-year.
...
select
  ...
  round(
    lb.entitlement * least(12, greatest(0,
      (extract(year from current_date) - extract(year from greatest(lb.leave_year_start, p.start_date))) * 12
      + (extract(month from current_date) - extract(month from greatest(lb.leave_year_start, p.start_date))) + 1
    )) / 12.0,
  2) as earned
